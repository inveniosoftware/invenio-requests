...
import { newOrIncreasedTotalHits } from "../../state/utils.js";
...
export const newOrAppendedPage = (state, parentRequestEventId, page, hits) => {
  ...
  if (Object.prototype.hasOwnProperty.call(commentReplies, page)) {
    return [...hits, ...commentReplies[page]];
  } else {
    return hits;
  }
};
...
const newCommentRepliesWithUpdate = (childComments, updatedComment, page) => {
  const newChildComments = _cloneDeep(childComments);
  const index = newChildComments[page].findIndex((c) => c.id === updatedComment.id);
  newChildComments[page][index] = updatedComment;
  return newChildComments;
};

const newCommentRepliesWithDelete = (childComments, deletedCommentId, page) => {
  const deletedComment = childComments[page].find((c) => c.id === deletedCommentId);
  return newCommentRepliesWithUpdate(
    childComments,
    {
      ...deletedComment,
      type: "L",
