...
export const TimelineEventBody = ({ payload, quoteReply, collapsedHeight = 200 }) => {
  ...
  if (!quoteReply) {
    return (
      <span ref={ref} className={`collapsible-comment ${isOverflowing && !expanded ? "overflowing" : ""}`} style={{ maxHeight }}>
        {content}
        {isOverflowing && !expanded && (
          <button className="show-more" onClick={toggleCollapsed}>Show more</button>
        )}
      </span>
    );
  }
