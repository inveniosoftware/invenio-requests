...
export const fetchTimeline = (focusEventId = undefined) => {
  return async (dispatch, getState, config) => {
    ...
    try {
      let firstPageResponse, lastPageResponse;

      if (focusEventId) {
        // Focused fetch: only firstPage
        firstPageResponse = await config.requestsApi.getTimelineFocused(focusEventId, {
          size,
        });
        ...
        const totalHits = firstPageResponse.data.hits.total;
        const lastPageNumber = Math.ceil(totalHits / size);

        if (lastPageNumber > 1) {
          // Multi-page: fetch last page immediately
          ...
          lastPageResponse = null; // Single-page timeline: no lastPage
        }
      }
