...
def test_threaded_comments_with_indexing(
    ...
    # Step 6: Verify parent has children_preview array with metadata
    assert (
        "children_preview" in parent_event
    ), "Parent event missing 'children_preview' field"
    assert (
        "children_count" in parent_event
    ), "Parent event missing 'children_count' field"
    assert (
        "has_more_children" in parent_event
    ), "Parent event missing 'has_more_children' field"

    children = parent_event["children_preview"]
    assert len(children) == 2, f"Expected 2 children, got {len(children)}"
    assert parent_event["children_count"] == 2, "Expected children_count to be 2"
    assert (
        parent_event["has_more_children"] is False
    ), "Expected has_more_children to be False"
    ...
    assert "children_preview" in parent_event
    children = parent_event["children_preview"]

    # Find the updated child in children_preview array
    ...
    ), "Parent's children_preview array not updated after child modification"


def test_delete_child_comment_preserves_threading(
    app, client_logged_as, headers, events_resource_data, example_request
):
    """Test deleting a child comment via REST API preserves threading structure."""
    client = client_logged_as("user1@example.org")
    request_id = example_request.id
    ...
    # Verify the deleted child is now a LogEvent
    response = client.get(
        f"/requests/{request_id}/comments/{reply1_id}", headers=headers
    )
    assert response.status_code == 200
    ...
    assert deleted_child["payload"]["event"] == "comment_deleted"
    # Verify threading preserved
    assert deleted_child["parent_id"] == parent_comment_id

    # Get timeline and verify parent's children_preview array
    response = client.get(f"/requests/{request_id}/timeline", headers=headers)
    assert response.status_code == 200
    hits = response.json["hits"]["hits"]
    ...
    assert parent_event is not None
    assert "children_preview" in parent_event
    assert parent_event["children_count"] == 2
    assert parent_event["has_more_children"] is False

    children = parent_event["children_preview"]
    assert len(children) == 2  # Both children still present

    # Verify one is deleted (LogEvent) and one is active (Comment)
    child_types = {child["type"] for child in children}
    assert LogEventType.type_id in child_types
    assert CommentEventType.type_id in child_types

    # Verify deleted child maintains parent_id
...
def test_nested_threading_not_allowed(
    app, client_logged_as, headers, events_resource_data, example_request
):
    """Test that 2-level threading (reply to reply) is NOT allowed."""
    client = client_logged_as("user1@example.org")
    request_id = example_request.id
    ...
    # The error message should indicate nested threading is not allowed
    error_message = response.json.get("message", "").lower()
    assert "nested" in error_message or "not allowed" in error_message

    # Step 4: Refresh index and verify only 1-level threading exists
    RequestEvent.index.refresh()
    ...
    assert (
        "children_preview" in parent_event
    ), "Parent should have children_preview field"
    assert (
        len(parent_event["children_preview"]) == 1
    ), "Parent should have exactly 1 child"
    assert parent_event["children_count"] == 1
    assert parent_event["has_more_children"] is False

    # Verify the child exists and has NO grandchildren
    child_event = parent_event["children_preview"][0]
    ...
    assert (
        "children_preview" not in child_event
        or len(child_event.get("children_preview", [])) == 0
    ), "Child should have no children (nested threading rejected)"
    ...
    parent_event = top_level_hits[0]
    assert (
        len(parent_event["children_preview"]) == 2
...
def test_children_preview_limit(
    ...
    """Test that children_preview respects the limit and has_more_children is set correctly."""
    client = client_logged_as("user1@example.org")
    request_id = example_request.id
    ...
    # Get timeline and check parent's children_preview
    response = client.get(f"/requests/{request_id}/timeline", headers=headers)
    assert response.status_code == 200
    hits = response.json["hits"]["hits"]
    parent_event = hits[0]
    ...
    assert "children_preview" in parent_event
    assert (
        len(parent_event["children_preview"]) == 5
    ), "Should only show 5 children in preview"

    # Verify metadata
    assert parent_event["children_count"] == 7, "Should count all 7 children"
    assert (
        parent_event["has_more_children"] is True
    ), "Should indicate more children exist"
...
def test_child_comment_permissions(
    ...
    # Check permissions on child in children_preview
    reply_in_preview = next(
        (c for c in parent_event["children_preview"] if c["id"] == reply_id), None
    )
...
def test_update_parent_with_children(
    ...
    assert len(parent_event["children_preview"]) == 2
    assert parent_event["children_count"] == 2
    assert parent_event["has_more_children"] is False

    # Verify child content is unchanged
    child_contents = [c["payload"]["content"] for c in parent_event["children_preview"]]
